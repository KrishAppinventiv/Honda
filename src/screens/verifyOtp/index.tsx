...
const VerifyOtp = () => {
  ...
  const [isValid, setIsValid] = useState<boolean[]>([true, true, true, true, true, true]);
  ...
  useEffect(() => {
    inputRefs.current[0]?.focus(); // ✅ No more 'focus' error
  }, []);
    ...
    <KeyboardAvoidingView
      ...
      <SafeAreaView style={{flex: 1}}>
        ...
        <View style={styles.bottomView}>
          <TouchableOpacity
            style={[styles.button, isOtpValid ? {} : styles.disabledButton]}
            disabled={!isOtpValid}
            onPress={handleSubmit}>
            <Text
              style={[
                styles.buttonText,
                {color: isOtpValid ? '#fff' : '#656565'},
              ]}>
              Verify
            </Text>
          </TouchableOpacity>
